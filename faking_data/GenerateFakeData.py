...
def genPath(num):
    ...
    source = (randint(0, GRID_WIDTH-1), randint(0, GRID_HEIGHT-1))
    ...
    for i in range(num):
        ...
        find_next_position = False
        while not find_next_position:
            if randint(0, 1) == 0:
                direction_x = -1 * randint(1, 10)
            else:
                direction_x = 1 * randint(1, 10)

            if randint(0, 1) == 0:
                direction_y = -1 * randint(1, 5)
            else:
                direction_y = 1 * randint(1, 5)

            new_x = path[previous_point_idx][0] + direction_x
            new_y = path[previous_point_idx][1] + direction_y
